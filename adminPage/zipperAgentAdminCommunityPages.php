...
class zipperAgentAdminCommunityPages extends zipperAgentAdminAbstractPage {
	...
	protected function getContent() {
		if($this->isUpdated()) {
			$title = zipperAgentUtility::getInstance()->getRequestVar("title");
			$cityZip = zipperAgentUtility::getInstance()->getRequestVar("cityZip");
			$propertyType = zipperAgentUtility::getInstance()->getRequestVar("propertyType");
			$bed = zipperAgentUtility::getInstance()->getRequestVar("bed");
			$bath = zipperAgentUtility::getInstance()->getRequestVar("bath");
			$minPrice = zipperAgentUtility::getInstance()->getRequestVar("minPrice");
			$maxPrice = zipperAgentUtility::getInstance()->getRequestVar("maxPrice");
		...
		<div style="float:left; padding-right: 40px;">
			...
			<form method="post">
				...
				<?php settings_fields(zipperAgentConstants::OPTION_GROUP_COMMUNITY_PAGES); ?>
		...
		<div style="float: left">
			...
			<?php $communityPageMenuItems = (array) zipperAgentMenu::getInstance()->getCommunityPagesMenuItems(); ?>
			<ul>
	...
	private function updateCommunityPages($title, $cityZip, $propertyType, $bed, $bath, $minPrice, $maxPrice) {
		$shortCode = zipperAgentShortcodeDispatcher::getInstance()->buildSearchResultsShortCode($cityZip, $propertyType, $bed, $bath, $minPrice, $maxPrice);
		$post = array(
		...
		zipperAgentMenu::getInstance()->addPageToCommunityPages($postId);
	}
