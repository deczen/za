...
class zipperAgentAgentBioWidget extends zipperAgentWidget {
	...
	public function widget($args, $instance) {
		...
		if($this->isEnabled($instance)) {
			...
			$agentPhotoUrl = get_option(zipperAgentConstants::AGENT_PHOTO_OPTION, null);
			$agentText = get_option(zipperAgentConstants::AGENT_TEXT_OPTION, null);
			$displayTitle = get_option(zipperAgentConstants::AGENT_DISPLAY_TITLE_OPTION, null);
			$contactPhone = get_option(zipperAgentConstants::CONTACT_PHONE_OPTION, null);
			$contactEmail = get_option(zipperAgentConstants::CONTACT_EMAIL_OPTION, null);
			$agentDesignations = get_option(zipperAgentConstants::AGENT_DESIGNATIONS_OPTION, null);
			$agentLicenseInfo = get_option(zipperAgentConstants::AGENT_LICENSE_INFO_OPTION, null);
			echo $beforeWidget;
			...
			<table>
				<tr>
					<?php if(!empty($agentPhotoUrl)) { ?>
						<td class="za-bio-img">
							<img id="za-bio-img" src="<?php echo $agentPhotoUrl; ?>" alt="<?php echo $displayTitle; ?>" />
						</td>
					<?php } ?>
					<?php if($displayType == zipperAgentAgentBioWidget::NARROW_DISPLAY_TYPE) { ?>
						</tr><tr>
					<?php } ?>
					<td>
						<div class="za-bio-about-info">
	...
	public function form($instance) {
		...
		<?php
		$configurationUrl = admin_url("admin.php?page=" . zipperAgentConstants::PAGE_BIO);
		?>
		<p>
