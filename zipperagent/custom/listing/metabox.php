...
add_action('wp_ajax_artifakt_delete_file', 'artifakt_delete_file');
function artifakt_delete_file() {
	if (!isset($_POST['data'])) return;
	list($post_id, $key, $attach_id, $src, $nonce) = explode('!', $_POST['data']);
	if (!wp_verify_nonce($nonce, 'artifakt_ajax_delete_file')) {
...
class ARTIFAKT_Meta_Box {
		...
		jQuery(document).ready(function($) {
			...
			$(".rw-delete-file").click(function(){
				var $parent = $(this).parent(),
					data = $(this).attr("rel");
				$.post(ajaxurl, {action: \'artifakt_delete_file\', data: data}, function(response){
					$parent.fadeOut("slow");
					alert(response);
				});
				return false;
	...
	function show() {
		...
		wp_nonce_field(basename(__FILE__), 'me_meta_box_nonce');
	...
	function show_field_file($field, $meta) {
		...
		if (!empty($meta)) {
			...
			$nonce = wp_create_nonce('me_ajax_delete_file');
	...
	function save($post_id) {
		...
		|| (!check_admin_referer(basename(__FILE__), 'me_meta_box_nonce'))		// verify nonce
		...
		foreach ($this->_fields as $field) {
			...
			// validate meta value
			if (class_exists('ME_Meta_Box_Validate') && method_exists('ME_Meta_Box_Validate', $field['validate_func'])) {
				$new = call_user_func(array('ME_Meta_Box_Validate', $field['validate_func']), $new);
			}
