...
}elseif(isset($single_property->vtlink)){
	
	if(is_array($single_property->vtlink)){
		$virtual_tours = $single_property->vtlink;
	}	
}elseif(isset($single_property->unmapped->{"Virtual Tour Url Unbranded"})){
	
	if(is_array($single_property->unmapped->{"Virtual Tour Url Unbranded"})){
		$virtual_tours = $single_property->unmapped->{"Virtual Tour Url Unbranded"};
	}else{
		$virtual_tours[] = $single_property->unmapped->{"Virtual Tour Url Unbranded"};
	}	
}
...
foreach( $virtual_tours as $virtual_index => $virtual_tour_url ):
	...
	if (stripos($virtual_tour_url, "iframe") !== false){ //iframe
		...
		$is_possible_popup = 0;
	} /* else if(stripos($virtual_tour_url, "exite-listings.com") !== false){
		$is_possible_popup = 0;
	}else if(stripos($virtual_tour_url, "video214.com") !== false){
		$is_possible_popup = 0;
	}else if(stripos($virtual_tour_url, "getopenframe.com") !== false){
		$is_possible_popup = 0;
	}else if(stripos($virtual_tour_url, "walkthruhomes.com") !== false){
		$is_possible_popup = 0;
	}else{ //normal url
