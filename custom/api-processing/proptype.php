<?php
	...
	$filename = 'proptype-' . date('M-d-Y').'.php';
	$cachefile = $dir.$filename;
	$filename_bak = 'proptype-' . date('M-d-Y', strtotime('-1 day', strtotime(date('M-d-Y')))).'.php';
	$cachefile_bak = $dir.$filename_bak;
	
	$files = scandir($dir);
	foreach($files as $file) {
		if( is_file( $dir . $file ) && $file!=$filename && $file!=$filename_bak && strpos($file, 'proptype-') !== false )
			@unlink( $dir . $file );
	}
