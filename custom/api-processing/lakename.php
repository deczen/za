<?php
	...
	$filename = 'lakename-' . date('M-d-Y').'.php';
	$cachefile = $dir.$filename;
	$filename_bak = 'lakename-' . date('M-d-Y', strtotime('-1 day', strtotime(date('M-d-Y')))).'.php';
	$cachefile_bak = $dir.$filename_bak;
	
	$files = scandir($dir);
	foreach($files as $file) {
		if( is_file( $dir . $file ) && $file!=$filename && $file!=$filename_bak && strpos($file, 'lakename-') !== false )
			@unlink( $dir . $file );
	}
	...
	$lakenames = isset($data->LAKECHAINNAME)?$data->LAKECHAINNAME:array();
	
	echo json_encode( $lakenames );
	/** CONTENT END **/
