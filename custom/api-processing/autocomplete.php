<?php
	...
	$filename_bak = 'autocomplete-' . date('M-d-Y', strtotime('-1 day', strtotime(date('M-d-Y')))).'.php';
	$cachefile_bak = $dir.$filename_bak;
	
	$files = scandir($dir);
	foreach($files as $file) {
		if( is_file( $dir . $file ) && $file!=$filename && $file!=$filename_bak && strpos($file, 'autocomplete-') !== false )
			@unlink( $dir . $file );
	}
