...
class zipperAgentLinkWidget extends zipperAgentWidget {
	
	public function __construct() {
		parent::__construct(
			"zipperAgentLinkWidget",
			"IDX: SEO City Links",
			array(
	...
	public function widget($args, $instance) {
		...
		if($this->isEnabled($instance)) {
			...
			$linkWidth = get_option(ZipperAgentConstants::SEO_CITY_LINK_WIDTH, null);
			echo $beforeWidget;
			$linkArray = get_option(zipperAgentConstants::SEO_CITY_LINKS_SETTINGS, null);
			if(!empty($linkArray)) {
				?>
				<div>
				<?php
				foreach($linkArray as $link) {
					$linkText = $link[zipperAgentConstants::SEO_CITY_LINKS_TEXT];
					$cityZip = $link[zipperAgentConstants::SEO_CITY_LINKS_CITY_ZIP];
					$propertyType = $link[zipperAgentConstants::SEO_CITY_LINKS_PROPERTY_TYPE];
					$minPrice = $link[zipperAgentConstants::SEO_CITY_LINKS_MIN_PRICE];
					$maxPrice = $link[zipperAgentConstants::SEO_CITY_LINKS_MAX_PRICE];
					if(!empty($linkText)) {
						$searchLinkInfo = new zipperAgentSearchLinkInfo($linkText, $cityZip, $propertyType, $minPrice, $maxPrice);
						$linkUrl = $this->createLinkUrl($searchLinkInfo);
						?>
						<div class="za-seo-link" style="
	...
	public function form($instance) {
		$instance = $this->migrate($instance);
		$configurationUrl = admin_url("admin.php?page=" . zipperAgentConstants::PAGE_SEO_CITY_LINKS);
		?>
		<p>
	...
	private function createLinkUrl($searchLinkInfo) {
		$resultsUrl = zipperAgentUrlFactory::getInstance()->getListingsSearchResultsUrl(true);
		$data = array();
