...
/**
 *
 * This singleton class is used to filter the content of zipperAgent VirtualPages.
 * We use the zipperAgentVirtualPageFactory class to retrieve the
 * proper VirtualPage implementation.
 *
 * @author zipperagent
 */
class zipperAgentVirtualPageDispatcher {
	...
	private function initialize() {
		...
		$type = get_query_var(zipperAgentConstants::ZA_TYPE_URL_VAR);
		...
		if(!$this->initialized && $postsCount === 0 && !empty($type)) {
			...
			$this->virtualPage = zipperAgentVirtualPageFactory::getInstance()->getVirtualPage($type);
			...
			//turn off some filters on za pages
			$this->removeFilters();
			$this->removeCaching();
		}
	}
	...
	/**
	 * We identify zipperAgent requests based on the query_var
	 * zipperAgentConstants::ZA_TYPE_URL_VAR.
	...
	public function getPageTemplate($pageTemplate) {
		...
		if($this->initialized) {
			...
			if(empty($virtualPageTemplate)) {
				$defaultVirtualPage = zipperAgentVirtualPageFactory::getInstance()->getVirtualPage(zipperAgentVirtualPageFactory::DEFAULT_PAGE);
	...
	/**
	 * For the za plugin page, we replace the content, with data retrieved from the zipperAgent servers.
	 *
	 * @param $content
	 */
	...
	/**
	 * For the za plugin page, we replace the excerpt, with data retrieved from the zipperAgent servers.
	 *
	 * @param $content
	 */
	...
	public function clearComments($comments) {
		if(get_query_var(zipperAgentConstants::ZA_TYPE_URL_VAR)) {
			$comments = array();
		}
		return $comments;
	}
